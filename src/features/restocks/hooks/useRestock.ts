...
import type { Restock, RestockQueryParams } from '@/interfaces/restock.interfce';
...
export function useRestocks(filters: RestockQueryParams = {}) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        editingRestock
          ? t('restocks.messages.updated_success', 'Reposicion actualizado correctamente')
          : t('restocks.messages.created_success', 'Reposicion registrado correctamente')
      );
  ...
  const openModal = (restock?: Restock) => {
    if (restock) {
