...
import type { RestockApiResponse, RestockItemApiResponse } from '../../interfaces/restock.response';
import { formatBs, formatDate, formatUsd } from '@/lib/format';

interface RestockDetailDialogProps {
  isOpen: boolean;
  onClose: () => void;
  restock: RestockApiResponse | null;
}
      ...
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader className="space-y-1">
          <DialogTitle className="flex flex-col gap-1 text-base sm:text-lg">
            <span>{t('restocks.detail_title', 'Detalle de Reabastecimiento')}</span>
        ...
        <div className="space-y-2.5">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
            {restock.items && restock.items.length > 0 ? (
              restock.items.map((item: RestockItemApiResponse, idx: number) => (
                <div
                  key={item.id ?? idx}
